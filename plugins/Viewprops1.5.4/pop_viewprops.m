...
if ~exist('fig','var')
    ...
    if ~typecomp && isfield(EEG.etc, 'ic_classification')
    	set(gcf,'Position', [1 1000-600/5*rows*1.2 800/7*column 600/5*rows*1.2]); % set(gcf,'Position', [pos(1) 20 800/7*column 600/5*rows*1.2]); % > > > > ESTO LO MODIFIQUE YO
    else
    	set(gcf,'Position', [1 1000-600/5*rows*1.2 800/7*column 600/5*rows]); % set(gcf,'Position', [pos(1) 20 800/7*column 600/5*rows]); % > > > > ESTO LO MODIFIQUE YO
    end
    incx = 120;
    incy = 110;
    sizewx = 100/column;
